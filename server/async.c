...
struct async_cancel
{
    ...
    obj_handle_t         wait_handle;         /* handle to wait for all the cancels to complete */
    struct process      *process;             /* referenced process, only used to close wait_handle */
};

static void async_cancel_dump( struct object *obj, int verbose );
static void async_cancel_satisfied( struct object *obj, struct wait_queue_entry *entry );
...
static const struct object_ops async_cancel_ops =
{
    ...
    NULL,                        /* remove_queue */
    NULL,                        /* signaled */
    async_cancel_satisfied,      /* satisfied */
    no_signal,                   /* signal */
    no_get_fd,                   /* get_fd */
...
static void async_cancel_satisfied( struct object *obj, struct wait_queue_entry *entry )
{
    ...
    close_handle( cancel->process, cancel->wait_handle );
...
static struct async_cancel *create_async_cancel( struct process *process )
{
    ...
    if (!(cancel->sync = create_event_sync( 1, 0 )))
    {
...
struct async
{
    ...
    struct async_cancel *async_cancel;    /* cancel object if async is being cancelled */
};
...
restart:
    LIST_FOR_EACH_ENTRY( async, &process->asyncs, struct async, process_entry )
    {
        ...
        {
            if (!async->cancelled) cancel_async( async );
            if (cancel)
            {
                assert( !async->async_cancel );
    ...
    if (cancel)
    {
        ...
        else cancel->wait_handle = alloc_handle( process, cancel, SYNCHRONIZE, 0 );
        *wait_handle = cancel->wait_handle;
    }
    return count;
}
