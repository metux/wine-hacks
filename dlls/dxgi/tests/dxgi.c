...
static void test_resize_fullscreen_window(IUnknown *device, BOOL is_d3d12, BOOL change_style)
{
    ...
    if (change_style)
    {
        ...
        {
            todo_wine ok(FALSE, "Currently ignoring window styles.\n");
            /* To avoid a success on the todo_wine inside check_fullscreen_state_() */
            expected_state.fullscreen_state.style = 0;
        }
    }
    else
    {
    ...
    if (change_style)
    {
        ...
        if (winetest_platform_is_wine)
        {
            ...
            GetClientRect(swapchain_desc.OutputWindow, &expected_state.fullscreen_state.client_rect);
        }
    }
    check_swapchain_fullscreen_state(swapchain, &expected_state);
    hr = IDXGISwapChain_GetDesc(swapchain, &swapchain_desc);
