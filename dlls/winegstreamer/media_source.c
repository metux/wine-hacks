...
static HRESULT media_source_create(struct object_context *context, IMFMediaSource **out)
{
    ...
    {
        /* Each MF stream holds a reference to the source, and that ref should be released
         * in Shutdown(). It is valid to create and release an MF source without ever
         * calling Start() and Shutdown(), and in that case, if streams take a source ref
         * on source creation, the refcount never reaches zero and Shutdown() is never
         * called. Therefore, streams are created in media_source_start(). The wg streams
         * are needed now to get the format and duration. Their buffer is freed in Start(). */
