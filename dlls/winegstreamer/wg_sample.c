...
HRESULT wg_transform_read_mf(wg_transform_t transform, IMFSample *sample,
        DWORD plane_size, DWORD *flags, bool *preserve_timestamps)
{
    struct wg_sample *wg_sample;
    IMFMediaBuffer *buffer;
    DWORD sample_size;
    ...
    {
        if (wg_sample->stride && plane_size)
        {
            /* The sample size must match the frame size, which differs from the contiguous length
             * if the buffer has extra width. MF allows a frame to be placed in a wider 2D buffer. */
            sample_size = min(plane_size, wg_sample->size);
        }
        else
        {
            if (wg_sample->stride)
                FIXME("Expected a plane size.\n");
