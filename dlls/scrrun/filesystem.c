...
static HRESULT WINAPI filesys_MoveFolder(IFileSystem3 *iface, BSTR src, BSTR dst)
{
    ...
    do
    {
        ...
        dst_folder = malloc((dst_len + 1 + name_len + 1) * sizeof(WCHAR));

        if (src_len)
            memcpy(src_folder, src, src_len * sizeof(WCHAR));
        memcpy(src_folder + src_len, find_data.cFileName, (name_len + 1) * sizeof(WCHAR));

        memcpy(dst_folder, dst, dst_len * sizeof(WCHAR));
        if (!separator)
            dst_folder[dst_len] = '\\';
        memcpy(dst_folder + dst_len + !separator, find_data.cFileName, (name_len + 1) * sizeof(WCHAR));
