...
static const char *debugstr_zip_entry(const struct zip_entry *entry)
{
    ...
    if (entry->opt + 1 < ARRAY_SIZE(opts))
        opt_str = opts[entry->opt + 1];
    else
        opt_str = wine_dbg_sprintf("%d", entry->opt);

    return wine_dbg_sprintf("{%s, %#I64x}", opt_str, entry->local_file_offset.QuadPart);
}
struct zip_part
{
...
static HRESULT compress_read_entries(IOpcFactory *factory, IStream *stream, ULONG dir_records, IOpcPartSet *partset)
{
    ...
    for (i = 0; i < dir_records; i++)
    {
        ...
        if (hr != S_OK)
            goto done;
        if (!(name = calloc(1, dir.name_length + 1)))
        {
            hr = E_OUTOFMEMORY;
            goto done;
        }

        hr = IStream_Read(stream, name, dir.name_length, &read);
        if (hr != S_OK)
        {
            free(name);
            goto done;
        ...
        if (!strcmp(name, content_types_name))
        {
            free(name);
        ...
        else
        {
            ...
            TRACE("Adding new OPC part %s: %s\n", debugstr_a(name), debugstr_zip_entry(&entry));

            len = MultiByteToWideChar(CP_ACP, 0, name, -1, NULL, 0);
            if (!(nameW = calloc(sizeof(WCHAR), len)))
            {
                hr = E_OUTOFMEMORY;
                free(name);
                goto done;
            ...
            free(name);
            hr = IOpcFactory_CreatePartUri(factory, nameW, &name_uri);
            free(nameW);
            if (FAILED(hr))
                goto done;
...
HRESULT compress_open_archive(IOpcFactory *factory, IStream *stream, IOpcPartSet *part_set)
{
    ...
    while (SUCCEEDED(hr))
    {
        ...
        if (FAILED(hr = IStream_Seek(stream, central_dir_start, STREAM_SEEK_SET, NULL)))
            goto next;
        return compress_read_entries(factory, stream, dir_records, part_set);
