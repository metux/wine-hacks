...
{
    UINT frame_width, frame_height, dst_width, dst_height, dst_size, src_stride, dst_stride, format_size, y;
    ...
    if (FAILED(hr = IWICBitmapLock_GetStride(lock, &dst_stride))
            || FAILED(hr = IWICBitmapLock_GetDataPointer(lock, &dst_size, &data)))
