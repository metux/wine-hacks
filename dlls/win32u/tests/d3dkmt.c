...
#define GET_DIM(test) ((test >> 4) & 7)
    ...
    for (UINT test = 0; test <= MAKETEST(7,7,0xf); test++)
    {
        ...
        C_ASSERT( width_cube * width_cube * width_cube * 4 == resource_size );
        C_ASSERT( width_1d * array_1d * 4 == resource_size );
        C_ASSERT( width_2d * height_2d * 4 == resource_size );
        C_ASSERT( width_3d * height_3d * depth_3d * 4 == resource_size );
