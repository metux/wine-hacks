...
static inline void append_lto_flags( struct strarray *args, int with_lto, int prefer_env )
{
    ...
    if (prefer_env > 0)
    {
        ...
        if (with_lto > 0)
        {
            ...
            static const char* _env_lto = NULL;

            if (!_init_lto)
            {
                _env_lto = getenv( "WINE_LTO_FLAGS" );
                if (_env_lto)
                    _env_lto = (_env_lto[0]) ? xstrdup( _env_lto ) : NULL;
                _init_lto = 1;
            }
            flags = _env_lto;
        }
        else
        {
            ...
            static const char* _env_lto_skip = NULL;

            if (!_init_lto_skip)
            {
                _env_lto_skip = getenv( "WINE_LTO_SKIP_FLAGS" );
                if (_env_lto_skip)
                    _env_lto_skip = (_env_lto_skip[0]) ? xstrdup( _env_lto_skip ) : NULL;
                _init_lto_skip = 1;
            }
            flags = _env_lto_skip;
        }
